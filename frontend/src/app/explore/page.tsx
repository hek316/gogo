    ...
    <div className="min-h-screen bg-bg">
      ...
      <div className="max-w-2xl mx-auto px-4 pt-10 pb-4">
        <div className="flex items-center gap-2 mb-1">
          <Compass size={18} strokeWidth={1} className="text-primary" aria-hidden="true" />
      ...
      <div className="sticky top-0 z-10 bg-bg/95 backdrop-blur-sm border-b border-border">
        <div className="max-w-2xl mx-auto px-4 pt-3 pb-3">
          {/* Sort tabs */}
          <div role="tablist" aria-label="정렬 기준" className="inline-flex bg-surface rounded-2xl p-1 gap-1 mb-3">
      ...
      <main className="max-w-2xl mx-auto px-4 pt-5 pb-28">
        {loading ? (
          ...
          <div className="text-center py-20">
            <div className="w-14 h-14 rounded-full bg-surface flex items-center justify-center mx-auto mb-4">
              <MapPin size={24} strokeWidth={1} className="text-primary" aria-hidden="true" />
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {filtered.map(place => {
              ...
              return (
                <div key={place.id} className="group bg-bg rounded-[20px] border border-border shadow-sm hover:-translate-y-0.5 hover:shadow-md transition-all duration-200 overflow-hidden" style={{ contentVisibility: 'auto', containIntrinsicSize: '0 180px' }}>
                  <Link href={`/places/${place.id}`} className="block">
                    <div className="relative h-[72px]">
                      {place.imageUrl ? (
                        <Image src={place.imageUrl} alt={place.name} fill className="object-cover" sizes="(max-width: 672px) 50vw, 320px" />
                      ) : (
        ...
        {!loading && filtered.length > 0 && (
          <div className="mt-8 bg-text-main rounded-[24px] px-6 py-5 text-center">
            ...
            <p className="text-white/75 text-xs mt-1">로그인 후 장소를 저장하고 좋아요를 눌러보세요</p>
            <Link
              href="/places"
