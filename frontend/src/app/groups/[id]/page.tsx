      ...
      <main className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        <div className="bg-surface rounded-[16px] p-4 flex items-center justify-between border border-border">
          ...
          <button onClick={copyInviteCode}
            className="bg-primary hover:bg-primary-hover text-text-on-primary font-medium text-sm px-4 py-2 rounded-[12px]">
            {copied ? '✓ 복사됨' : '복사'}
          </button>
        </div>
        ...
        <div className="bg-bg-secondary rounded-[20px] p-6 border border-border shadow-sm">
          ...
          {group.members.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-2">
              {group.members.map(m => (
                <span key={m.id} className="bg-surface text-primary text-sm px-3 py-1 rounded-full font-medium">
                  {m.nickname}
                </span>
              ))}
            </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setShareModal(true)}
              className="text-sm bg-primary hover:bg-primary-hover text-text-on-primary px-4 py-1.5 rounded-[16px] font-medium flex items-center gap-1">
          ...
          {groupPlaces.length === 0 ? (
            ...
            <div className="space-y-3">
              {groupPlaces.map(gp => (
                <div key={gp.id} className="bg-bg-secondary rounded-[20px] p-6 border border-border hover:shadow-md transition-all duration-300">
